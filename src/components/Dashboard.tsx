...
export default function Dashboard({ selectedDriverId }: { selectedDriverId?: string }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 md:px-10 py-3 md:py-16 space-y-3 md:space-y-20">
      ...
      {/* Stats Grid - Mobile: 3 cards in row, Desktop: all cards */}
      <div className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-4 gap-1.5 md:gap-4">
        {filteredStatCards.map((card, i) => (
          <div 
            key={i} 
            className={`bg-white p-2.5 md:p-6 rounded-xl md:rounded-2xl border border-zinc-200/60 shadow-[0_2px_8px_rgba(0,0,0,0.04)] flex flex-col gap-1 md:gap-3 ${!card.showOnMobile ? 'hidden md:flex' : ''}`}
          >
            ...
            {card.subtitle && (
              <p className="text-[12px] text-emerald-600 font-medium hidden md:block">{card.subtitle}</p>
            )}
          </div>
        ))}
      </div>
