...
app.get("/api/transactions", (req, res) => {
  ...
  if (rickshaw_id) { query += " AND t.rickshaw_id = ?"; params.push(rickshaw_id); }
  if (driver_id) { query += " AND t.driver_id = ?"; params.push(driver_id); }
  query += " ORDER BY t.date DESC, t.id DESC";
  if (limit) { query += " LIMIT ?"; params.push(parseInt(limit as string)); }
    ...
    db.transaction(() => {
      ...
      if (oldTx && oldTx.driver_id) {
        ...
        if ((oldTx.category === 'rent_pending' || oldTx.type === 'pending') && (!oldTx.amount || oldTx.amount === 0) && oldTx.notes) {
          ...
          if (amountMatch) oldFinalAmount = parseFloat(amountMatch[1]);
        }
...
app.get("/api/categories", (req, res) => {
  ...
  try {
    ...
    let params: any[] = [];
    if (type) { query = "SELECT * FROM categories WHERE type = ? ORDER BY name"; params = [type]; }
...
app.delete("/api/categories/:id", (req, res) => {
  ...
  try {
    ...
    if (category.is_default) return res.status(400).json({ error: "Cannot delete default category" });
    const usageCount = db.prepare("SELECT COUNT(*) as count FROM transactions WHERE category = ?").get(category.name) as { count: number };
    if (usageCount.count > 0) return res.status(400).json({ error: `Used in ${usageCount.count} tx(s)` });
...
app.get("/api/settings", (req, res) => {
  try { res.json(db.prepare("SELECT * FROM settings WHERE id = 1").get() || {}); }
  catch (error: any) { res.status(500).json({ error: error.message }); }
});
...
app.post("/api/settings", (req, res) => {
  ...
  try {
    db.prepare("INSERT OR REPLACE INTO settings (id, currency, currency_symbol, date_format, auto_backup, report_format) VALUES (1, ?, ?, ?, ?, ?)")
      .run(currency, currencySymbol, dateFormat, autoBackup ? 1 : 0, reportFormat);
    res.json({ success: true });
  } catch (error: any) { res.status(400).json({ error: error.message }); }
});
...
app.get("/api/stats", (req, res) => {
  ...
  try {
    let driverFilter = ""; let params: any[] = [];
    if (driver_id) { driverFilter = " AND driver_id = ?"; params.push(driver_id); }
    ...
    if (driver_id) {
      totalInvestment = db.prepare("SELECT SUM(r.investment_cost) as total FROM rickshaws r JOIN rickshaw_assignments a ON r.id = a.rickshaw_id WHERE a.driver_id = ? AND a.end_date IS NULL").get(driver_id) as any;
    } else {
      totalInvestment = db.prepare("SELECT SUM(investment_cost) as total FROM rickshaws").get() as any;
    }
    const monthlyData = db.prepare(`SELECT strftime('%Y-%m', date) as month, SUM(CASE WHEN type = 'income' AND category != 'rent_pending' THEN amount ELSE 0 END) as income, SUM(CASE WHEN type = 'expense' AND category != 'rent_pending' THEN amount ELSE 0 END) as expense FROM transactions WHERE 1=1 ${driverFilter} GROUP BY month ORDER BY month ASC LIMIT 12`).all(...params);
    const dailyData = db.prepare(`SELECT date, SUM(CASE WHEN type = 'income' AND category != 'rent_pending' THEN amount ELSE 0 END) as income, SUM(CASE WHEN type = 'expense' AND category != 'rent_pending' THEN amount ELSE 0 END) as expense FROM transactions WHERE date >= date('now', '-30 days') ${driverFilter} GROUP BY date ORDER BY date ASC`).all(...params);
    let pendingBalance = { total: 0 };
    if (driver_id) { pendingBalance = db.prepare("SELECT pending_balance as total FROM drivers WHERE id = ?").get(driver_id) as any; }
    else { pendingBalance = db.prepare("SELECT SUM(pending_balance) as total FROM drivers").get() as any; }
    res.json({
